...
describe("BTC utils", () => {
    it('parse Mainnet P2PKH', () => {
        ...
        expect(parseBtcAddress({address, network: BtcNetwork.Mainnet})).toEqual(BtcAddressType.P2pkh);
    });

    it('parse Testnet P2PKH', () => {
        const address = 'mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn';

        expect(parseBtcAddress({address, network: BtcNetwork.Testnet})).toEqual(BtcAddressType.P2pkh);
    });
    ...
    it('fails on invalid P2PKH', () => {
        ...
        expect(() => parseBtcAddress({address, network: BtcNetwork.Mainnet})).toThrow();
        expect(() => parseBtcAddress({address, network: BtcNetwork.Testnet})).toThrow();
        expect(() => parseBtcAddress({address, network: BtcNetwork.Regtest})).toThrow();
    });

    it('parse Mainnet P2SH', () => {
        const address = '3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy';

        expect(parseBtcAddress({address, network: BtcNetwork.Mainnet})).toEqual(BtcAddressType.P2sh);
    });

    it('parse Testnet P2SH', () => {
        const address = '2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc';

        expect(parseBtcAddress({address, network: BtcNetwork.Testnet})).toEqual(BtcAddressType.P2sh);
    });

    it('parse Regtest P2SH', () => {
        const address = '2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc';

        expect(parseBtcAddress({address, network: BtcNetwork.Regtest})).toEqual(BtcAddressType.P2sh);
    });
    ...
    it('parse Mainnet Bech32 P2WPKH', () => {
        const [a, b] = ['bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4', 'bc1q973xrrgje6etkkn9q9azzsgpxeddats8ckvp5s'];

        expect(parseBtcAddress({address: a, network: BtcNetwork.Mainnet})).toEqual(BtcAddressType.P2wpkhV0);
        expect(parseBtcAddress({address: b, network: BtcNetwork.Mainnet})).toEqual(BtcAddressType.P2wpkhV0);
    });

    it('parse Testnet Bech32 P2WPKH', () => {
        const address = 'tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx';

        expect(parseBtcAddress({address, network: BtcNetwork.Testnet})).toEqual(BtcAddressType.P2wpkhV0);
    });

    it('parse Regtest Bech32 P2WPKH', () => {
        const address = 'bcrt1q6z64a43mjgkcq0ul2znwneq3spghrlau9slefp';

        expect(parseBtcAddress({address, network: BtcNetwork.Regtest})).toEqual(BtcAddressType.P2wpkhV0);
    });
