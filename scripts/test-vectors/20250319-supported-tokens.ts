...
const getCustomTokens = async (
  ...
  for (const ledgerId of ledgerIds) {
    try {
      console.log(`Connecting to ledger ${ledgerId}...`);
      const ledger = Actor.createActor(icrcLedgerIdl.idlFactory, {
        agent,
      ...
      console.log(`Successfully added ${metadata.symbol} to supported tokens`);
    } catch (error) {
      console.error(`Error fetching metadata for ${ledgerId}:`, error);
    }
  }

  console.log(`Total custom tokens found: ${supportedTokens.length}`);
  return supportedTokens;
};
...
const main = async () => {
  try {
    ...
    const customTokens = await getCustomTokens(customLedgerCanisterIds);
